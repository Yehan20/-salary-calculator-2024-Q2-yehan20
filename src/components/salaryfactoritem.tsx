...
const SalaryFactoritemStyled = styled.div`
 ...
 ${Andriod({justifyContent:"flex-start"})}
...
const SalaryFactoritem = ({id,name,amount,checked,type}:SalaryFactorObj) => {
  ...
  return (
     <SalaryFactoritemStyled key={id}>
           ...
           <h5 className={`text-xs  flex items-center gap-3 ${type}`}> {checked ?<FaCheck color='blue'/>:""} EPF/ETF <span className='text-gray-500'>|</span>  </h5>
